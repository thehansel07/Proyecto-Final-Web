...
/**
 * Functional component for displaying lighting,
 * as well as containing the functions for deleting and editing lights.
 * Editing is done via a Modal connected to each light, thereby having that lights 
...
const Lighting = (props) => {
    ...
    const delLighting = () => {
        let id = props.id
        axios.delete("https://localhost:5001/lights/" + id);
        setShow(false);
        alert("Lighting deleted!");
        window.location.reload();
    }

    const editLighting = ( event ) => {
        ...
        for(var key in updateLighting) {
            if(updateLighting[key] === undefined) {
                updateLighting[key] = props[key];
            } 
            if(Object.is(updateLighting[key], NaN)) {
                updateLighting[key] = Number.parseInt(props[key], 10)
            }
        }

        axios.put("https://localhost:5001/lights", updateLighting);
        setShow(false);
        alert("Lighting updated! Refresh page");
    }
            ...
            <div className="card shadow-sm">
                ...
                <div className="card-body">
                    <div className="row">
                        ...
                        <small className="card-text col-6 text-right">Nunca <span className="text-muted">{props.price},-</span></small>
                    </div>
                </div>
                <div className="card-footer text-right">
                    <div className="row justify-content-between pl-2 pr-2">
                        <Button className="col-2" variant="danger" onClick={delLighting}><span className="fa fa-trash-o"></span></Button>
        ...
        <Modal key={`modal-${props.id}`}  show={show} onHide={hideModal} animation={false}>
            ...
            <Modal.Body>
                <Form onSubmit={editLighting}>
                    ...
                    <Form.Label>Descripcio: </Form.Label>
                    <Form.Control onChange={handleChange} name="denscription" type="text" placeholder={props.description} className="mb-2" />
                    <Form.Row>
            ...
            <Modal.Footer>        
                <Button className="mr-auto" variant="danger" onClick={delLighting}>Borrar</Button>
